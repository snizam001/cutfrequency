...
make_option(c("-b", "--bam2"), 
	    ...
	    metavar="character"),
make_option(c("-c", "--bam3"), 
	    type="character",
	    default=NA,
	    help="name of bam file - spikeIn: experiment (without .bam extension)", 
	    metavar="character"),
make_option(c("-d", "--bam4"), 
	    type="character", 
	    default=NA,
	    help="name of bam file - spikeIn: control (without .bam extension)", 
	    metavar="character"),
make_option(c("-m", "--motifFile"),
	    ...
	    metavar="character"),
make_option(c("-p", "--peakFile"),
	    type="character",
	    default=NA,
	    help="If the number of the motifs are higher in the genome and do not have enough memory, then might be interested in exploring cut-frequency at only peaks. If yes then, provide peakFile in the bed format.", 
	    metavar="character"),
make_option(c("-x", "--centreOfmotif"), 
	    type="integer",
	    help="Desired centre of motif on positive DNA strand",
	    metavar="numeric"),
make_option(c("-s", "--script_folder"),
	    type="character", 
	    default=NA,
	    help="Folder location with other assessory scripts of this tool", 
	    metavar="character"),
...
f=paste("perl ",myfolder,"/filePreparation.pl ", bam1," ", mymotif , " ", mythread, " ", peakFile, sep="" )
x = system(f)
...
f=paste("perl ",myfolder,"/filePreparation.pl ", bam2," ", mymotif, " " , mythread, " ", peakFile, sep="" )
x = system(f)
...
f=paste("perl ",myfolder,"/filePreparation.pl ", bam3," ", mymotif, " " , mythread, " ", peakFile, sep="" )
x = system(f)
if (x != 0)
	{ 
		stop()
...
f=paste("perl ",myfolder,"/filePreparation.pl ", bam4," ", mymotif, " " , mythread, " ", peakFile, sep="" )
x = system(f)
if (x != 0)
	{ 
		stop()
